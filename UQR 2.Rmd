...
analysis_data <- data_modifiable %>%
  ...
  filter(!is.na(hours), hours > 0, hours < 97) %>%
  mutate(
...
-------------------------------------------------------------------------
DÉCOMPOSITION OAXACA-BLINDER : ÉVOLUTION TEMPORELLE (2010s vs 1990s)
---------------------------------------------------------------------

```{r}
cat("\n=== DÉCOMPOSITION TEMPORELLE (2011 vs 2019) ===\n")

# 1. Nettoyage des "faux" NA de l'EU-LFS et filtre temporel
data_clean <- analysis_data_corrected %>%
  mutate(
    # Remplacement des codes d'erreur EU-LFS (-3, -2, -1) par de vrais NA
    sector = ifelse(as.numeric(as.character(sector)) < 0, NA, as.character(sector)),
    age = ifelse(age < 0, NA, age)
  ) %>%
  # On compare deux dates aux nomenclatures sectorielles et professionnelles stables
  filter(year %in% c(2011, 2019)) %>%
  mutate(period_recent = ifelse(year == 2019, 1, 0)) %>%
  mutate(female_num = as.numeric(as.character(female))) %>%
  drop_na(rif_gap_new, period_recent, age, age_sq, female_num, sector, country)

# 2. Support commun (On ne garde que les Pays et Secteurs présents aux 2 époques)
valid_countries <- data_clean %>%
  group_by(country) %>% filter(n_distinct(period_recent) == 2) %>% pull(country) %>% unique()

valid_sectors <- data_clean %>%
  group_by(sector) %>% filter(n_distinct(period_recent) == 2) %>% pull(sector) %>% unique()

oaxaca_data_time <- data_clean %>%
  filter(country %in% valid_countries) %>%
  filter(sector %in% valid_sectors) %>%
  mutate(
    country = droplevels(as.factor(country)),
    sector = droplevels(as.factor(sector))
  )

cat("\nObservations valides en 2011 :", sum(oaxaca_data_time$period_recent == 0), "\n")
cat("Observations valides en 2019 :", sum(oaxaca_data_time$period_recent == 1), "\n")

# 3. Formule de base (comme demandée par l'encadrant, le sexe est à droite)
formula_str <- "rif_gap_new ~ age + age_sq + female_num + sector + country"

# Note : Si vous avez finalement extrait la profession (occupation), décommentez ceci :
# formula_str <- paste(formula_str, "+ occupation")

formula_str <- paste(formula_str, "| period_recent")
cat("\nFormule du modèle :", formula_str, "\n\n")

# 4. Exécution du modèle
oaxaca_time_results <- oaxaca(
  formula = as.formula(formula_str),
  data = oaxaca_data_time,
  R = 50 # Passez à 100 pour la version finale si votre ordinateur est assez puissant
)

print(summary(oaxaca_time_results))
```

-------------------------------------------------------------------------
VISUALISATION DE LA DÉCOMPOSITION TEMPORELLE (AVEC REGROUPEMENT)
-------------------------------------------------------------------------
...
# 1. Extraction des résultats macro
val_total <- oaxaca_time_results$y$y.diff
val_expl <- oaxaca_time_results$twofold$overall[5, "coef(explained)"]
val_unexpl <- oaxaca_time_results$twofold$overall[5, "coef(unexplained)"]

df_macro <- data.frame(
  Composante = factor(
    c("1. Baisse totale de l'écart (2011-2019)", 
      "2. Effet de Composition (Démographie)", 
      "3. Effet de Structure (Marché du travail)"),
    levels = c("3. Effet de Structure (Marché du travail)", 
               "2. Effet de Composition (Démographie)", 
               "1. Baisse totale de l'écart (2011-2019)")
  ),
  Valeur = c(val_total, val_expl, val_unexpl)
)

# --- GRAPHIQUE 1 : MACRO ---
g_macro <- ggplot(df_macro, aes(x = Composante, y = Valeur, fill = Valeur > 0)) +
  geom_bar(stat = "identity", width = 0.5, color = "black", alpha = 0.8) +
  geom_text(aes(label = sprintf("+%.2f h", Valeur),
  ...
  labs(
    title = "Décomposition globale de l'évolution (2011 - 2019)",
    subtitle = "Qu'est-ce qui explique la réduction de la polarisation horaire ?",
    y = "Contribution (en heures)"
  ) +
  # CORRECTION : Marge dynamique au lieu de limites fixes
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.3)))

print(g_macro)

# 2. Préparation et Regroupement des variables (Micro)
df_micro_brut <- as.data.frame(oaxaca_time_results$twofold$variables[[5]])
df_micro_brut$Variable <- rownames(df_micro_brut)

df_micro_grouped <- df_micro_brut %>%
  filter(Variable != "(Intercept)") %>%
  mutate(
    Categorie = case_when(
      grepl("age", Variable) ~ "Âge (Vieillissement)",
      grepl("female", Variable) ~ "Genre (Féminisation)",
      grepl("sector", Variable) ~ "Secteurs d'activité (Tertiarisation)",
      grepl("country", Variable) ~ "Pays (Hétérogénéité géographique)",
      TRUE ~ "Autre"
    )
  ) %>%
  group_by(Categorie) %>%
  summarise(
    Composition = sum(`coef(explained)`),
    Structure = sum(`coef(unexplained)`)
  )

# --- GRAPHIQUE 2 : EFFET DE COMPOSITION ---
g_comp <- ggplot(df_micro_grouped, aes(x = reorder(Categorie, Composition), y = Composition, fill = Composition > 0)) +
  geom_bar(stat = "identity", width = 0.6, color = "black", alpha = 0.8) +
  geom_text(aes(label = sprintf(ifelse(Composition > 0, "+%.2f", "%.2f"), Composition),
                hjust = ifelse(Composition > 0, -0.2, 1.2)), size = 4.5) +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#0072B2", "FALSE" = "#D55E00")) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "none", plot.title = element_text(face = "bold"), axis.title.y = element_blank()) +
  labs(
    title = "Détail de l'effet de composition (+0.38h)",
    subtitle = "Impact de l'évolution démographique et structurelle de l'emploi",
    y = "Heures"
  ) +
  # CORRECTION : Ajustement automatique avec 25% de marge
  scale_y_continuous(expand = expansion(mult = c(0.25, 0.25)))

print(g_comp)

# --- GRAPHIQUE 3 : EFFET DE STRUCTURE ---
g_struc <- ggplot(df_micro_grouped, aes(x = reorder(Categorie, Structure), y = Structure, fill = Structure > 0)) +
  geom_bar(stat = "identity", width = 0.6, color = "black", alpha = 0.8) +
  geom_text(aes(label = sprintf(ifelse(Structure > 0, "+%.2f", "%.2f"), Structure),
                hjust = ifelse(Structure > 0, -0.2, 1.2)), size = 4.5) +
  coord_flip() +
  scale_fill_manual(values = c("TRUE" = "#0072B2", "FALSE" = "#D55E00")) +
  theme_minimal(base_size = 14) +
  theme(legend.position = "none", plot.title = element_text(face = "bold"), axis.title.y = element_blank()) +
  labs(
    title = "Détail de l'effet de structure (+2.84h)",
    subtitle = "Impact des nouvelles dynamiques du marché du travail",
    y = "Heures"
  ) +
  # CORRECTION : Ajustement automatique avec 20% de marge
...
#ggsave("Oaxaca_Macro_Time.png", plot = g_macro, width = 10, height = 5, dpi = 300, bg = "white")
#ggsave("Oaxaca_Comp_Time.png", plot = g_comp, width = 10, height = 6, dpi = 300, bg = "white")
#ggsave("Oaxaca_Struc_Time.png", plot = g_struc, width = 10, height = 6, dpi = 300, bg = "white")
```
