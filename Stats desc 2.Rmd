...
ggsave("evolution_heures_travail-par-groupe-age.png",
       plot = p,
       width = 8,      # largeur en pouces
       height = 5,     # hauteur en pouces
       dpi = 300,      # résolution élevée
...
# Calcul des tendances par groupe d'âge
age_trends <- age_hours %>%
  group_by(age_group) %>%
  summarise(
    `Période` = paste(min(`Year of survey`), "-", max(`Year of survey`)),
    `Moyenne` = round(mean(mean_hours), 1),
    `Tendance (h/an)` = round(lm(mean_hours ~ `Year of survey`)$coefficients[2], 3),
    `Tendance (h/décennie)` = round(lm(mean_hours ~ `Year of survey`)$coefficients[2] * 10, 1),
    `p-value` = round(summary(lm(mean_hours ~ `Year of survey`))$coefficients[2,4], 4),
    `R²` = round(summary(lm(mean_hours ~ `Year of survey`))$r.squared, 4),
    `Observations` = sum(n),
    .groups = "drop"
  ...
  mutate(
    `Significativité` = ifelse(`p-value` < 0.001, "***",
                              ifelse(`p-value` < 0.01, "**",
                                    ifelse(`p-value` < 0.05, "*", "NS")))
  ) %>%
  select(`Groupe d'âge` = age_group, Période, Moyenne, `Tendance (h/an)`, 
         `Tendance (h/décennie)`, Significativité, `p-value`, `R²`, Observations)
...
```{r}
# Création et sauvegarde du tableau en PNG

age_table_grob <- tableGrob(age_trends, 
                           rows = NULL,
                           theme = ttheme_minimal(
                             base_size = 10,
                             padding = unit(c(4, 4), "mm"),
...
# Sauvegarde
ggsave("tendances_heures_par_age_tableau.png", 
       plot = age_table_grob, 
       width = 12, 
       height = 5, 
       dpi = 300, 
...
```{r}
# Graphique
p_gini <- ggplot(inequality_hours, aes(x = `Year of survey`)) +
...
ggsave("evolution_inegalites_heures_gini.png", p_gini, width = 8, height = 5, dpi = 300, bg = "white")
```

```{r}
# Créer un tableau synthétique
summary_table <- data.frame(
  Indicateur = c("Gini moyen", "Gini 1983", "Gini 2022", "Évolution Gini", 
                 "Tendance Gini (an)", "Significativité",
                 "CV moyen", "CV 1983", "CV 2022", "Évolution CV",
                 "Tendance CV (an)", "Significativité",
                 "Corrélation Gini-CV"),
  Valeur = c(
...
# Créer et sauvegarder le tableau
...
# Ajouter un titre
title <- textGrob("Synthèse des inégalités d'heures travaillées (1983-2022)",
                  gp = gpar(fontsize = 12, fontface = "bold"),
                  x = 0, hjust = 0)

final_table <- arrangeGrob(title, table_grob, ncol = 1, heights = c(0.1, 0.9))

ggsave("tableau_synthese_inegalites-gini.png", 
       plot = final_table, 
       width = 10, 
       height = 6, 
       dpi = 300, 
...
```{r}
# Graphique
p_excessif <- ggplot(overwork_analysis, aes(x = `Year of survey`, y = prop_overwork)) +
...
ggsave("evolution_travail_excessif_sup48.png", p_excessif, width = 8, height = 5, dpi = 300, bg = "white")
...
```{r}
# Graphique 1 : Évolution par secteur avec palette cohérente
if (nrow(sector_hours) > 0) {
  
  # Définir une palette cohérente pour les 8 secteurs
  # Palette "Set3" de RColorBrewer (qualitative, 12 couleurs max, convient pour catégories)
  library(RColorBrewer)
  ...
  p_sectors <- ggplot(sector_hours, aes(x = `Year of survey`, y = mean_hours, color = sector_name)) +
    geom_line(linewidth = 1.2) +
    geom_point(size = 2.5) +
    scale_color_brewer(palette = "Set2", name = "Secteur économique") +
    labs(
      title = "Évolution des heures travaillées hebdomadaires par secteur (2008-2022)",
      subtitle = "Données EU-LFS, classification NACE Rev 2 agrégée",
      x = "Année",
      y = "Heures hebdomadaires moyennes",
      caption = "Source: EU Labour Force Survey 2023\nNote: Exclut les codes spéciaux (-3, -2, -1) du champ NACE"
    ) +
  ...
  ggsave("evolution_heures_par_secteur_2008_2022.png",
         plot = p_sectors,
         width = 10,
         height = 6,
         dpi = 300,
  ...
  cat("Graphique sauvegardé sous : evolution_heures_par_secteur_2008_2022.png\n")
