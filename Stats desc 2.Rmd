...
# 1. Calculer la moyenne des heures par année (et par trimestre si voulu)
...
Evolution des heures de travail en fonction du quintile :
...
```{r}
# Calculer l'évolution par année et quintile
hours_by_income_corrected <- data_hours_income_corrected %>%
  group_by(`Year of survey`, income_quintile) %>%
  summarise(
    mean_hours = mean(`Number of hours usually worked, main job`, na.rm = TRUE),
    n = n(),
    se_hours = sd(`Number of hours usually worked, main job`, na.rm = TRUE) / sqrt(n),
    .groups = "drop"
  )

# Graphique
ggplot(hours_by_income_corrected,
  ...
  scale_color_manual(values = c("Q1 (0-20%)" = "red",
                               "Q2 (20-40%)" = "orange",
                               "Q3 (40-60%)" = "green",
                               "Q4 (60-80%)" = "blue",
                               "Q5 (80-100%)" = "purple")) +
  scale_fill_manual(values = c("Q1 (0-20%)" = "red",
                              "Q2 (20-40%)" = "orange",
                              "Q3 (40-60%)" = "green",
                              "Q4 (60-80%)" = "blue",
                              "Q5 (80-100%)" = "purple")) +
...
On observe une diminution bien plus forte dans le premier quintile (deux fois plus forte que dans la classe moyenne 3e quintile)
...
```{r}
# Graphique
ggplot(sex_hours, aes(x = `Year of survey`, y = mean_hours, color = sex_label)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3) +
  labs(
    title = "Évolution des heures travaillées par sexe",
    subtitle = "Heures hebdomadaires moyennes",
    x = "Année",
    y = "Heures moyennes",
    color = "Sexe"
  ) +
